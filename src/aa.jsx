...
export default function App() {
  ...
  useEffect(() => {
    const storedCartIcon = JSON.parse(localStorage.getItem("addToCartIcon"));
    const storedTheBestIcon = JSON.parse(localStorage.getItem("addToTheBestIcon"));
    const storedAddTheBestComponent = JSON.parse(localStorage.getItem("addTheBestComponent"));
    const storedAddCartComponent = JSON.parse(localStorage.getItem("addCartComponent"));
    
    if (storedCartIcon) setAddToCartIcon(storedCartIcon);
    ...
    if (storedAddTheBestComponent) setAddTheBestComponent(storedAddTheBestComponent);
    if (storedAddCartComponent) setAddCartComponent(storedAddCartComponent);
  }, []);
  ...
  const handleAddToCartClick = (productId, product) => {
    setAddToCartIcon(prev => prev + 1);
    setAddCartComponent(prev => ({ ...prev, [productId]: product }));
  };

  const handleTheBestComponent = (productId, product) => {
  ...
  const handleSubmit = (e) => {
    ...
    const isValid = /^\w+@(gmail|com|yahoo|outlook)\.(com|org|edu|sa)$/.test(email) &&
    ...
    <div className="popup-overlay">
      <div className="popup-container">
        <div className="popup-content">
          ...
          <button className="cta-btn" onClick={() => handleAddToCartClick(selectedProduct.id, selectedProduct)}>
            Add to Cart
          </button>
        ...
        <form className="payment-form" onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="form-label">
              ...
              {!email ? (
                ...
                <span className="warning">Missing @ symbol</span>
          ...
          <div className="form-group">
            <label className="form-label">
              ...
              />
              {!nameOnCard ? (
                <span className="hint-nameof-card">Enter your Name of card</span>
              ) : nameOnCard.length < 5 ? (
            ...
            <div className="credit-card-group">
              <label className="form-label">
                ...
                <div className="credit-card-input-wrapper">
                  ...
                  <img src={CreditCard} alt="credit-card" className="credit-card-icon" />
                </div>
              </label>
            </div>
          </div>
          <div className="checkout-btns">
            ...
            <button type="button" onClick={removeCheckOut} className="closecheckout-btn">No Thanks</button>
          </div>
        </form>
      </div>
  ...
  }, [addToCartIcon, addToTheBestIcon, addTheBestComponent, addCartComponent]);
